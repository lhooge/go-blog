...
//MySQLUserDatasource providing an implementation of UserDatasourceService using MariaDB
type MySQLUserDatasource struct {
	SQLConn *sql.DB
}

//List returns a list of user
func (rdb MySQLUserDatasource) List(p *Pagination) ([]User, error) {
...
func (rdb MySQLUserDatasource) Get(userID int) (*User, error) {
	var u User
...
func (rdb MySQLUserDatasource) GetByMail(mail string) (*User, error) {
	var u User

	if err := rdb.SQLConn.QueryRow("SELECT id, is_admin, active, display_name, username, email, salt, password FROM user WHERE email=? AND active=true ", mail).
...
func (rdb MySQLUserDatasource) GetByUsername(username string) (*User, error) {
	var u User

	if err := rdb.SQLConn.QueryRow("SELECT id, is_admin, active, display_name, username, email, salt, password FROM user WHERE username=? AND active=true ", username).
		Scan(&u.ID, &u.IsAdmin, &u.Active, &u.DisplayName, &u.Username, &u.Email, &u.Salt, &u.Password); err != nil {
		return nil, err
	}
	return &u, nil
...
func (rdb MySQLUserDatasource) Create(u *User) (int, error) {
...
func (rdb MySQLUserDatasource) Update(u *User, changePassword bool) error {
	var stmt bytes.Buffer
...
func (rdb MySQLUserDatasource) Count(ac AdminCriteria) (int, error) {
	var stmt bytes.Buffer

	stmt.WriteString("SELECT count(id) FROM user ")

	if ac == OnlyAdmins {
		stmt.WriteString("WHERE is_admin = true")
	} else if ac == NoAdmins {
...
//Remove removes the user by id
func (rdb MySQLUserDatasource) Remove(userID int) error {
	var stmt bytes.Buffer
